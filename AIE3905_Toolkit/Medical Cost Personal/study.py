...
TITLE = "Medical Insurance Cost XAI"
SUBTITLE = "Regression: explain recorded insurance charges"
LEARNING_GOAL = "Practice explanation and subgroup error analysis on a sensitive healthcare-cost dataset without confusing prediction with clinical or pricing justification."
CONTEXT = "The model predicts `charges` from age, BMI, children, smoking status, sex, and region. It is a classroom exercise only. It must not be used for underwriting, clinical assessment, or automated pricing."
LOCAL_INTERPRETATION = "A local explanation shows how this historical model combines the input variables. It cannot explain a person's true health needs or justify a charge."
GLOBAL_INTERPRETATION = "Smoking status and BMI may be highly predictive in this dataset. Their importance is not a policy recommendation and should be interpreted alongside equity concerns."
AUDIT_TITLE = "Sex-group error audit"
AUDIT_EXPLANATION = "Regression MAE and mean predictions are compared by sex to reveal whether the model has noticeably different error patterns."
AUDIT_CAUTION = "Sex is sensitive in many contexts. Subgroup metrics are diagnostic only and cannot establish fairness by themselves."
...
def load_data() -> pd.DataFrame:
    return pd.read_csv(ROOT / "insurance.csv")
