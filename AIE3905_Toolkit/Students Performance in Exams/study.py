...
TITLE = "Student Math Performance XAI"
SUBTITLE = "Regression: explain mathematics scores from pre-test background variables"
LEARNING_GOAL = "Examine predictive performance and group error carefully when education data contains demographic and socioeconomic proxies."
CONTEXT = "The target is `math score`. Reading and writing scores are deliberately excluded because they are measured in the same exam setting and would make this an outcome-profiling model rather than an early-support example."
LOCAL_INTERPRETATION = "A local explanation shows model associations for one student profile. It must not be used to label ability, make high-stakes placement decisions, or infer potential."
GLOBAL_INTERPRETATION = "Feature importance shows what the model uses in this dataset, not what a student can or cannot change."
AUDIT_TITLE = "Gender-group error audit"
AUDIT_EXPLANATION = "Regression MAE is reported by gender. Learners should also inspect the source data and discuss whether protected attributes belong in the model at all."
AUDIT_CAUTION = "Small datasets and social proxies can produce misleading patterns. Human educators must retain decision responsibility."
...
def load_data() -> pd.DataFrame:
    return pd.read_csv(ROOT / "StudentsPerformance.csv")
