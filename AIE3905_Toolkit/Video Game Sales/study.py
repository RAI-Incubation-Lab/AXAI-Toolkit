...
TITLE = "Video Game Sales XAI"
SUBTITLE = "Regression: explain global sales from release context"
LEARNING_GOAL = "Practice leakage-aware target design by predicting global sales without using regional sales, which sum directly into the target."
CONTEXT = "The model predicts `Global_Sales` in millions from release year, platform, genre, and publisher. Regional sales columns and rank are deliberately excluded because they would leak the outcome."
LOCAL_INTERPRETATION = "A local explanation shows why the model expects a particular sales level for one historical release. It is not a forecast of a future game's commercial success."
GLOBAL_INTERPRETATION = "Publisher and platform capture historical market structure. High importance does not mean a publisher or genre causes sales."
AUDIT_TITLE = "Genre error audit"
AUDIT_EXPLANATION = "Regression error is compared by genre to reveal whether the model is more reliable for some game categories than others."
AUDIT_CAUTION = "Rare genres have few examples and can have unstable error estimates. This dataset also reflects historical reporting and regional coverage."
...
def load_data() -> pd.DataFrame:
    df = pd.read_csv(ROOT / "vgsales.csv")
    df["Year"] = pd.to_numeric(df["Year"], errors="coerce")
    return df.dropna(subset=[TARGET]).copy()


def train() -> TabularRun:
    return train_tabular(load_data(), target=TARGET, task="regression", numeric_features=NUMERIC, categorical_features=CATEGORICAL, time_column="Year")


def audit_groups(run: TabularRun) -> pd.Series:
    return run.X_test["Genre"]
